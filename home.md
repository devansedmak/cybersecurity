...
Now let's open a connection to the SSH service with the command `ssh vagrant@10.0.2.4`. We insert the obtained password and we now have a SSH session, where we impersonate the user _vagrant_. By inserting the command `cmd` we get a _Command Prompt_.
